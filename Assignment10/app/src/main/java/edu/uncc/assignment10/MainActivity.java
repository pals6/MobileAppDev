...
public class MainActivity extends AppCompatActivity implements BillsFragment.BillsListener,
    ...
    public void onSortSelected(String sortAttribute, String sortOrder) {
        ...
        if (sortAttribute.equals("Date") && sortOrder.equals("ASC")) {
            Collections.sort(mBills, (bill1, bill2) -> bill1.getBillDate().compareTo(bill2.getBillDate()));
        } else if (sortAttribute.equals("Date") && sortOrder.equals("DESC")) {
            Collections.sort(mBills, (bill1, bill2) -> bill2.getBillDate().compareTo(bill1.getBillDate()));
        } else if (sortAttribute.equals("Category") && sortOrder.equals("ASC")) {
            Collections.sort(mBills, (bill1, bill2) -> bill1.getCategory().compareTo(bill2.getCategory()));
        } else if (sortAttribute.equals("Category") && sortOrder.equals("DESC")) {
            Collections.sort(mBills, (bill1, bill2) -> bill2.getCategory().compareTo(bill1.getCategory()));  // Corrected line
        } else if (sortAttribute.equals("Discount") && sortOrder.equals("ASC")) {
            Collections.sort(mBills, (bill1, bill2) -> Double.compare(bill1.getDiscount(), bill2.getDiscount()));
        } else if (sortAttribute.equals("Discount") && sortOrder.equals("DESC")) {
            Collections.sort(mBills, (bill1, bill2) -> Double.compare(bill2.getDiscount(), bill1.getDiscount()));
        }
        getSupportFragmentManager().popBackStack();
    }
